...
export default async function GroupPage() {
  // The admin workspace, not the member passbook: members are sent to /denied.
      ...
      <DashboardPage
        ...
        stats={[
          ...
          },
          {
            ...
            value: formatPaise(overview.monthlyExpectedPaise, { whole: true }),
            icon: Banknote,
            hint: `${formatPaise(overview.collectedThisMonthPaise, { whole: true })} ${t.common.collected}`,
          },
          {
            label: t.group.outOnLoan,
            value: formatPaise(overview.outstandingPrincipalPaise, { whole: true }),
          ...
          {cycle ? (
            <dl className="mt-4 grid gap-x-6 gap-y-3 text-sm sm:grid-cols-2">
              ...
              <Rule
                label={t.group.fines}
                value={
                  cycle.distributeFines ? t.common.sharedWithMembers : t.common.keptInCorpus
                }
              />
              <Rule
                label={t.group.availableToLend}
                value={formatPaise(overview.availableFundsPaise, { whole: true })}
              />
            </dl>
          ) : (
